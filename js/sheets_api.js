/**
 * sheets_api.js
 * 운영협업포털 — Google Sheets 연동 모듈
 */
const SheetsAPI = (() => {
  const cfg = () => window.PORTAL_CONFIG || {};
  const SHEET_GID = { 일정:0, 영향페이지:1, 결함유형:2, 운영링크:3, 이관항목:4 };
  ...
  async function appendRow(sheetName, rowData) {
    ...
    const res = await fetch(
      ...
      { method:'POST', headers:{ 'Authorization':`Bearer ${GITHUB_TOKEN}`, 'Accept':'application/vnd.github+json', 'Content-Type':'application/json' },
        body: JSON.stringify({ ref:'main', inputs:{ sheet_name:sheetName, row_json:JSON.stringify(rowData) } }) }
    );
    if (res.status===204) return { ok:true, message:'저장 요청 완료' };
